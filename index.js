...
function findElementGreaterThan(arr, num) {
  ...
  let element = arr.find((elem) => elem > num)
  if (element > num) {
    // Перевіряємо чи є елемент який задовільняє умову
    return element //Якщо так повертаємо елемент
  }

  //   не працюючий варіант:
  //   arr.find((elem) => {
  //     if (elem > num) {
  //       return elem  // повертає  undefined
  //     }
  else {
...
function findIndexByProperty(arr, name) {
  ...
  if (result !== -1) {
    ...
    return -1 // Повертаємо знайдений індекс або -1, якщо об'єкт не знайдено.
  }
}
...
function getArrayKeys(arr) {
  ...
  //     arrayKeys[arrayKeys.length] = elem
  //   }
...
function getPositiveArrayValues(arr) {
  ...
  const arrValues = Array.from(iterator)
  // За допомогою методу filter() отримуємо масив лише з додатніми значеннями.
  const positiveArrayValues = arrValues.filter(
    (elem) => elem > 0,
  )
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  //   const flattenSquare = flatten.map((x) => x * x)
  // За допомогою методу filter() фільтруємо елементи, які більше за порогове значення.
  //   const filteredSquaredArray = flattenSquare.filter(
  //     (elem) => elem > threshold,
  //   )
...
function expandArrayByN(arr, n) {
  ...
  // Мій ваіант:
  // *******************************************************************************************************
  // *                !!!                   Костиль. потребую допомоги.               !!!                  *
  // *******************************************************************************************************
  let extend = []
  const stringArray = arr.flatMap(
    (x) =>
      (extend = Array.from(
        x.toString().repeat(n), // Запхав у рядок, бо не зрозумів як заповнити масив у циклі for
      ).concat()),
  )
  const expandedArray = stringArray.map((item) =>
    parseInt(item),
  )
...
function findLongestWord(arr) {
  ...
  const longestCount = arr.reduce(
    (biggest, word) =>
      biggest < Array.from(word).length
  ...
  // *                !!!                   Костиль. потребую допомоги.               !!!                  *
  // *******************************************************************************************************
  const longestWord = arr.find(
    (word) => Array.from(word).length === longestCount,
  )
